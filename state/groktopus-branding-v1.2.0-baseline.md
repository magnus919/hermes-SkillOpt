...
- **Differentiate from existing covers** — before selecting demographics, list the existing cover assets in `assets/` to see which subjects are already represented. Choose subjects from groups NOT YET in the collection. If the skill already has a Pattern D cover with a Black woman in her 40s, the next Pattern D cover should use a different age, race, or gender to expand the brand's visual range. Rotate through unrepresented demographics.
...
> **Before choosing cover demographics:** Check the existing cover assets in `assets/` for subjects already represented. The collection includes multiple covers with specific subject types. Select a demographic group NOT YET in the collection to expand the brand's visual diversity.
...
**If the article touches multiple patterns:** Identify the PRIMARY thesis — the article's core argument, not its secondary themes or supporting examples. Map the PRIMARY thesis to the pattern using the table above. If the thesis is structural critique (how a system produces negative outcomes), it's Pattern C territory even if the secondary impact is burnout. If still ambiguous, the higher-energy pattern (C or B over A or D) is usually the safer default for the groktopus brand's dramatic register.
...
> **If the article also needs inline diagrams, charts, or process illustrations**, see the **Inline Illustrations & Data Visualization** section below. The inline style uses the same vintage steel engraving technique on parchment background, with cross-hatched fills, navy serif labels in Bodoni-style typography, and thin navy border framing. Do NOT use cover-style composition for inline diagrams — they have their own templates and rules.
