...
Templates: `blank`, `title-card`, `video-edit`. 
- `blank` — general purpose, custom compositions (choose this unless you have a specific reason for another)
- `title-card` — animated title sequences and opening credits
- `video-edit` — video editing / timeline-based media compositions
Pass `--example <name>` to pick one, `--video clip.mp4` or `--audio track.mp3` to seed with media.
...
If your composition uses a font that isn't in the compiler's auto-resolve list (the compiler warns `No deterministic font mapping for: <font>`), you have three options:

**Option A — Google Fonts CDN (easiest):** Add a `<link>` or `@import` to Google Fonts in your HTML `<head>`. The compiler will fetch and embed the font files automatically:
```html
<link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&display=swap" rel="stylesheet">
```

**Option B — Local `@font-face` (most reliable):** Download a `.woff2` file and place it in `assets/fonts/`. Declare in your CSS:
```css
@font-face {
...
```
The compiler will skip inlining for local files — the font loads from the project directory at render time.

**Option C — Fallback substitution (fastest):** Use a similar mapped font (e.g., `Impact` for condensed bold, `Georgia` for serif) and document the substitution. The compiler will render without warnings, but the visual result may differ from the intended design.
...
- **Audio:** always a separate `<audio>` element (video is `muted playsinline`). For background music (BGM), the element must have **both** an `id` attribute and a `src` attribute directly on the `<audio>` tag — a `<source>` child alone triggers `media_missing_src` at render time:
  ...
  **Dual Audio (Narration + BGM) pattern:** When a composition has both TTS narration and background music, reduce BGM volume so narration is clearly audible:

  ```bash
  # Generate TTS narration (full volume)
  npx hyperframes tts "Script text" --voice af_nova --output assets/narration.wav
  # Pre-process BGM to 15% volume
  ffmpeg -i source-bgm.mp3 -filter:a "volume=0.15" assets/bgm.mp3
  ```

  Both audio elements inside the composition div with separate data-track-index values. Example:

  ```html
  <audio id="narration" src="assets/narration.wav" data-start="0" data-duration="2.5" data-track-index="5"></audio>
  <audio id="bgm" src="assets/bgm.mp3" data-start="0" data-duration="12.0" data-track-index="0" data-bgm autoplay loop></audio>
  ```

  See [references/background-music.md](references/background-music.md) for volume control details.
...
#### Render Config Quick Reference

| Decision | Options | When to Use |
|----------|---------|-------------|
| `--quality` | `draft` | Fast iteration, testing animation timing — skips anti-aliasing for speed |
| | `standard` | Team review, internal approval — balanced quality/speed |
| | `high` | **Final delivery** — full anti-aliasing, multi-sampling. Use for client-facing or published output |
| `--format` | `mp4` | **Default.** Universally compatible. No transparency support. |
| | `webm` | **Transparency needed** — overlay videos, OBS compositing, green-screen replacements. VP9 codec with alpha channel. |
| `--docker` | On | **Byte-identical reproducibility** needed. Use for client deliverables where re-renders must match exactly. Requires Docker daemon running — verify with `docker info` before relying on this flag. |
| | Off (default) | Fast iteration, personal use, or when Docker daemon isn't available. Falls back to standard Chrome pipeline. For approximate reproducibility, pin the `hyperframes` npm version in `package.json`. |
| `--fps` | 30 | Default. Good balance of quality and render speed. |
| | 60 | **High-motion content** (sports, fast-paced animation). Doubles render time and file size. |
| | 24 | **Film-style content.** Slower motion, lower file size. |

#### Lint Error Recovery Workflow

When `npx hyperframes lint` reports errors, follow this structured recovery cycle:

1. **Read the error.** Each error includes: the element ID/name, the error code (e.g., `timed_element_missing_clip_class`), the consequence (what will break), and the fix.
2. **Fix the issue.** Apply the fix the lint message describes — add `class="clip"`, fix `data-track-index` overlap, register the missing timeline, etc.
3. **Re-run lint.** `npx hyperframes lint --strict` to confirm the fix resolved the error. Fix any remaining issues in the same cycle.
4. **Proceed only on clean lint.** Do not skip to validate, inspect, or render until lint reports 0 errors.
5. **Run the full pipeline:** `npx hyperframes validate && npx hyperframes inspect && npx hyperframes render --quality draft --output draft.mp4`

Common lint errors and their fixes:

| Error Code | What It Means | Fix |
|------------|--------------|-----|
| `timed_element_missing_clip_class` | A timed element (has `data-start`/`data-duration`) lacks `class="clip"`. The element will be visible for the entire composition. | Add `class="clip"` to the element alongside any existing classes. |
| `overlapping_clips_same_track` | Two clips with the same `data-track-index` have overlapping time windows. | Give each clip a unique `data-track-index`, or stagger their `data-start`/`data-duration` to avoid overlap. |
| `gsap_css_transform_conflict` | CSS transform property conflicts with GSAP tween on the same element. | Remove CSS `transform`/`translate` declarations and use GSAP's `x`/`y`/`scale` aliases instead. |
| `media_missing_id` | An `<audio>` or `<video>` element lacks an `id` attribute. | Add `id="descriptive-name"` to the media element. |
| `media_missing_src` | An `<audio>` element's `src` is on a `<source>` child instead of the `<audio>` tag. | Move `src="assets/file.mp3"` to the `<audio>` element itself. |
