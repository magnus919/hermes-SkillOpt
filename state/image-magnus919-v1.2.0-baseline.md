...
**Subject derivation process — getting from thesis to subject:**

1. **Extract the thesis.** What is the article's core argument in one sentence? (e.g., "LLMs pattern-match rather than reason, and we misrecognize their cognition because we use the wrong benchmarks.")
2. **Identify the friction point.** What is coming apart, colliding, or transforming in that thesis? (e.g., "Misrecognition — measuring one thing as if it were another.")
3. **Brainstorm 3 concrete visual metaphors.** Write three different ways to MAKE the friction point visible. Each one should be a specific scene with objects in spatial relationships. (e.g., a shadow being measured instead of the object casting it; a mirror reflecting something that isn't in front of it; a map being used as if it were the territory.)
4. **Pick the strongest.** The one that best captures the thesis at a glance and rewards a second look.
5. **Describe it with concrete objects.** Write the one-line SUBJECT using specific nouns and spatial relationships. No abstractions — the reader should be able to picture every element.
...
#### Path A Error Recovery

| Symptom | Likely Cause | Fix |
|---------|-------------|-----|
| Safety system rejection | Prompt named a public figure or trademarked entity | Rephrase to describe the scene without naming the subject. See Pitfalls section for workaround. |
| Image_generate returns timeout (no result) | Server-side processing delay | Retry. If persistent, simplify the SUBJECT line — fewer elements reduces processing time. |
| Output is blank or all black | Prompt too abstract with no concrete objects | Rewrite SUBJECT with specific nouns and spatial relationships. No abstractions. |
| Output doesn't match the SUBJECT at all | gpt-image-2 cannot follow precise structural specs | Simplify the subject geometry. Use broader metaphors (see Pitfalls: gpt-image-2 cannot render precise object shapes). |
| First generation is ~90% right, one element wrong | Model didn't render a specific detail correctly | Use Iterative Refinement (below) — edits API on own output, or structured regeneration with keep/change prompt. |
...
See `references/photo-to-gonzo.md` for the complete recipe including a full bash implementation and an error recovery table covering missing model parameter, wrong model name, timeout, and invalid image errors.
...
- A reference image exists that is **thematically relevant** to the article's subject. The reference's subject, composition, and emotional register should map to what the cover needs to communicate. Do NOT use Path B just because a file exists — if the reference is irrelevant (e.g., a pet photo for an article about cloud infrastructure), the gonzo conversion will inject an unrelated subject into the composition.
...
| Situation | Path | Why |
|-----------|------|-----|
| No reference image exists | **A** — text-to-image via `image_generate` | Generate from concept. SUBJECT line describes the scene. |
| Reference exists AND thematically relevant | **B** — edits API with reference | Preserve subject's likeness/composition while converting to gonzo style. |
| Reference exists BUT thematically irrelevant to article | **A** — ignore the reference, generate from concept | Forcing an irrelevant reference into the composition produces a thematically incoherent cover. |
| First Path A generation is ~90% right, one element wrong | **Iterative Refinement** — edits API on own output | Fix the single wrong element (hand position, missing detail) while preserving the entire scene. Do not regenerate from scratch. Max 1-2 cycles before drift. |
...
**Fallback — when the edits API is not available as a tool:** Use structured regeneration with a keep/change prompt pattern. Construct a new prompt that explicitly states what to preserve from the first generation and what to change:

```
KEEP: same composition, same colors, same background scene, same mood
CHANGE: REMOVE the [wrong element], REPLACE with [correct element]
```

This is less precise than the edits API (it regenerates rather than inpainting) but preserves the overall composition better than regenerating from scratch. Max 2 cycles before drift.
...
#### Cover Format Decision Table (single-post quick reference)

| Situation | Action |
|-----------|--------|
| Existing cover is gonzo-style but wrong size (not 1600px) or format (PNG) | **Resize.** Convert to 1600px JPEG Q85. Update frontmatter to `cover: cover.jpg`. Remove old file. |
| Existing cover is non-gonzo (photorealistic, vector, cartoon) | **Regenerate.** Run Path B (edits API with reference image) or Path A (if no reference matches). Show result to user. Resize to 1600px JPEG Q85. |
| Frontmatter uses absolute path (`cover: /images/old-cover.png`) | **Update.** Change to relative path: `cover: cover.jpg`. Move/convert the image file into the post bundle if needed. |
| Frontmatter uses nonstandard filename (`cover.webp`, `image.png`) | **Standardize.** Rename to `cover.jpg`, convert to JPEG Q85, update frontmatter. Remove old file. |
| Multiple cover files in post directory (`cover.png` + `cover.jpg`) | **Deduplicate.** Keep the standard one (`cover.jpg`), remove any alternatives. Verify frontmatter matches. |
| No cover file exists in post directory but frontmatter references one | **Investigate.** Check `static/images/` for absolute-path references, or check git history for the original file. Generate new cover if unresolvable. |
...
### Series Patterns (3+ articles on the same subject)

For multi-article series where a single subject undergoes a transformation across installments, use the **triptych series cover pattern** — see `references/triptych-series-cover-pattern.md`. The pattern creates a visual arc across covers: same subject cropped differently per article, material-shifted (organic → electronic → hybrid) to mirror the argumentative arc. Best when each article reveals a different facet of the same core subject.
